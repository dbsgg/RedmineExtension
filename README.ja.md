...
- **API アクセスキー**（任意）— 個人の Redmine REST API キー。空のままなら公開サーバーを
  匿名（読み取り専用）で閲覧できます。非公開プロジェクトの閲覧やステータス変更・コメント追加には
  必要です。キーは **Windows 資格情報マネージャー**（汎用資格情報、ユーザーごとに DPAPI 暗号化）
  に保存され、平文では保持されません。
...
[Microsoft PowerToys](https://learn.microsoft.com/windows/powertoys/install) と **Command Palette** モジュールの有効化が必要です。以下のいずれかでインストールできます。

- **Extension Gallery（いちばん簡単）:** コマンドパレットの設定 → **拡張機能** から Extension Gallery を開き、「**Redmine**」をインストール。
...
コマンドパレットが起動中だった場合は、インストール後に **再読み込み（Reload）** を一度実行してください。以後の更新は Store 経由で自動配信されます。
